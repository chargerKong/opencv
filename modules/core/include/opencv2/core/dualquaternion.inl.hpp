...
DualQuat<T> DualQuat<T>::gdqblend(InputArray _dualquat, InputArray _weight, QuatAssumeType assumeUnit)
{
    ...
    CV_CheckTypeEQ(CV_MAT_DEPTH(_dualquat.type()), CV_MAT_DEPTH(cv::traits::Type<T>::value), "");
    Mat dualquat = _dualquat.getMat(), weight = _weight.getMat();
    Size dq_s = dualquat.size();
    ...
    if (dualquat.channels() != 8)
    {
        CV_Error(Error::StsBadArg, "The channel of dualquat should be 8");
    }
